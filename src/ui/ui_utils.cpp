...
lv_obj_t* createLabel(const lv_font_t* font, lv_color_t textColor, int xOfs, int yOfs, 
  ...
  lv_obj_set_style_text_font(label, font, 0);  // 设置字体
  lv_obj_set_style_text_color(label, textColor, 0);  // 设置文字颜色
  lv_obj_set_width(label, 320);  // 设置宽度
  // 如果指定了高度，则设置高度
  if (height > 0) {
  ...
  lv_obj_align(label, LV_ALIGN_TOP_MID, xOfs, yOfs);  // 设置对齐方式
  // 设置长文本模式
  if (wrap) {
...
lv_obj_t* createImage(const void* src, int width, int height, int xOfs, int yOfs,lv_color_t bgColor) {
  ...
  lv_img_set_src(img, src);  // 设置图像源
  lv_obj_set_size(img, width, height);  // 设置尺寸
  lv_obj_set_pos(img, xOfs, yOfs);  // 设置对齐方式和偏移
  lv_obj_set_style_bg_color(img, bgColor, 0);
  lv_obj_set_style_bg_opa(img, 0, 0);
  lv_obj_add_flag(img, LV_OBJ_FLAG_HIDDEN);  // 设置隐藏标志
  return img;
}